...
static ipcmessage *ready_usb_intr_req_queue_data[8] ATTRIBUTE_ALIGN(32);
static int ready_usb_intr_req_queue_id;
static ipcmessage *pending_usb_intr_req_queue_data[8] ATTRIBUTE_ALIGN(32);
static int pending_usb_intr_req_queue_id;

static int ensure_init_oh1_context(void);

/*static void print_ipcmessage(const ipcmessage *msg)
{
	svc_printf("command: 0x%x\n", msg->command);
	svc_printf("result: 0x%x\n", msg->result);
	svc_printf("fd: 0x%x\n", msg->fd);
	svc_printf("ioctlv.command: 0x%x\n", msg->ioctlv.command);
	svc_printf("ioctlv.num_in: 0x%x\n", msg->ioctlv.num_in);
	svc_printf("ioctlv.num_io: 0x%x\n", msg->ioctlv.num_io);
	svc_printf("ioctlv.vector: %p\n", msg->ioctlv.vector);
	svc_printf("ioctlv.vecto[0].data: %p\n", msg->ioctlv.vector[0].data);
	svc_printf("ioctlv.vecto[0].len: 0x%x\n", msg->ioctlv.vector[0].len);
	svc_printf("ioctlv.vecto[1].data: %p\n", msg->ioctlv.vector[1].data);
	svc_printf("ioctlv.vecto[1].len: 0x%x\n", msg->ioctlv.vector[1].len);
	svc_printf("ioctlv.vecto[2].data: %p\n", msg->ioctlv.vector[2].data);
	svc_printf("ioctlv.vecto[2].len: 0x%x\n", msg->ioctlv.vector[2].len);
}*/

static int configure_usb_intr_msg_handed_to_oh1(u32 fd, u16 min_wLength)
{
	u16 *p_wLength = usb_intr_msg_handed_to_oh1.ioctlv.vector[1].data;
	u32 *p_len = &usb_intr_msg_handed_to_oh1.ioctlv.vector[2].len;
	void **p_data = &usb_intr_msg_handed_to_oh1.ioctlv.vector[2].data;
	...
	usb_intr_msg_handed_to_oh1.fd = fd;
...
{
	u16 src_wLength  = *(u16 *)src->ioctlv.vector[1].data;
	u16 dst_wLength  = *(u16 *)dst->ioctlv.vector[1].data;
	void *src_data   = src->ioctlv.vector[2].data;
	void *dst_data   = dst->ioctlv.vector[2].data;

	assert(dst_wLength >= src_wLength);

	memcpy(dst_data, src_data, src_wLength);
	os_sync_after_write(dst_data, src_wLength);

	return src_wLength;
}

static int handle_oh1_dev_intr_message(ipcmessage *recv_msg, ipcmessage **ret_msg,
				       u8 bEndpoint, u16 wLength, void *data)
{
	int ret = 0;
	...
	if (bEndpoint == EP_HCI_EVENT) {
		...
		ret = os_message_queue_receive(ready_usb_intr_req_queue_id, &ready_msg,
					       IOS_MESSAGE_NOBLOCK);
		if (ret == IOS_OK) {
			u16 size = copy_ipcmessage_bulk_intr_ioctlv_data(recv_msg, ready_msg);
			ret = os_message_queue_ack(recv_msg, size);
		} else {
			/* Hand down to OH1 a "private" message  to fill it from the real USB */
			ret = configure_usb_intr_msg_handed_to_oh1(recv_msg->fd, wLength);
			if (ret == IOS_OK) {

				*ret_msg = &usb_intr_msg_handed_to_oh1;
				/* Push the received message to the PendingQ */
				ret = os_message_queue_send(pending_usb_intr_req_queue_id,
...
static int OH1_IOS_ResourceReply_hook(ipcmessage *ready_msg, int retval)
{
	...
	if (ready_msg == &usb_intr_msg_handed_to_oh1) {
		...
		ret = os_message_queue_receive(pending_usb_intr_req_queue_id, &pending_msg,
					       IOS_MESSAGE_NOBLOCK);
		if (ret == IOS_OK) {
			/* Copy message data */
			u16 size = copy_ipcmessage_bulk_intr_ioctlv_data(pending_msg, ready_msg);
			ret = os_message_queue_ack(pending_msg, size);
		} else {
			/* Push message to ReadyQ */
			ret = os_message_queue_send(ready_usb_intr_req_queue_id,
						    ready_msg, IOS_MESSAGE_NOBLOCK);
		}
	} else {
		ret = os_message_queue_ack(ready_msg, retval);
	}
...
static int ensure_init_oh1_context(void)
{
	...
	if (!initialized) {
		...
		ret = os_message_queue_create(ready_usb_intr_req_queue_data,
					      ARRAY_SIZE(ready_usb_intr_req_queue_data));
		if (ret < 0)
			return ret;
		ready_usb_intr_req_queue_id = ret;

		ret = os_message_queue_create(pending_usb_intr_req_queue_data,
					      ARRAY_SIZE(pending_usb_intr_req_queue_data));
		if (ret < 0)
			return ret;
		pending_usb_intr_req_queue_id = ret;
